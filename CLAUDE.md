...
### 2026-03-04: Importar PageShell como default, no como named
- **Error**: Importar `{ PageShell }` causaba build error porque es un default export.
- **Fix**: Cambiar a `import PageShell from '@/components/magazine/PageShell'`.
- **Aplicar en**: Cualquier página nueva que use `PageShell`.

### 2026-03-04: Eliminar links a /contacto al quitar la página
- **Error**: Mantener links a `/contacto` en `SiteHeader` y `SiteFooter` causaba errores de traducción y links rotos.
- **Fix**: Eliminar los links y sus respectivos `t('contacto')` / `t('contact')` en los componentes.
- **Aplicar en**: Siempre que se elimine una ruta principal, limpiar Header y Footer.
