          ...
          <div className="hidden md:flex border-b border-gray-200">
            {tabs.map((tab, index) => (
              <button
                ...
                className={`flex-1 py-4 px-6 text-body-md font-semibold text-center transition-colors relative ${activeTab === index
                  ? 'text-teal-600'
            ...
            {tabs.map((tab, index) => (
              <div key={index} className="border border-gray-200 rounded-xl overflow-hidden">
                <button
                  ...
                  className={`w-full py-4 px-5 text-left font-semibold text-body-md flex items-center justify-between transition-colors ${activeTab === index ? 'bg-teal-50 text-teal-700' : 'bg-white text-gray-700'
                    }`}
                >
                  {t(`items.${index}.title`)}
                  <svg
