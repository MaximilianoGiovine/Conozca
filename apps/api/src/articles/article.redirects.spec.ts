...
describe('ArticleService redirects on slug change', () => {
  ...
  it('creates redirect when slug changes', async () => {
    const id = 'a1';
    const old = { id, slug: 'old-slug', authorId: 'au', categoryId: 'c', editorId: 'e', publishedAt: null };
    const updated = {
      ...old,
      slug: 'new-slug',
      author: { id: 'au', name: 'A' },
    ...
    const res = await service.update(id, { slug: 'new-slug' } as any, 'e', Role.ADMIN);

    expect(res.slug).toBe('new-slug');
    expect((prisma as any).redirect.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ fromSlug: 'old-slug', toSlug: 'new-slug', articleId: id }) })
    );
  });
});
