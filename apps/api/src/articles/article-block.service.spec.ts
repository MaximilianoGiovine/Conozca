...
describe('ArticleService - Blocks', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ArticleService,
        {
          ...
          useValue: {
            $transaction: jest.fn(async (fn) => fn({ articleBlock: articleBlockMock })),
            article: {
              findUnique: jest.fn(),
              create: jest.fn(),
              update: jest.fn(),
  ...
  describe('createBlock', () => {
    it('should create a block for an article when user is editor', async () => {
      ...
      jest.spyOn((prismaService as any).articleBlock, 'findFirst').mockResolvedValue(null);

      jest.spyOn((prismaService as any).articleBlock, 'create').mockResolvedValue(mockBlock);
    ...
    it('should set correct order for new block', async () => {
      ...
      jest.spyOn((prismaService as any).articleBlock, 'create').mockResolvedValue({
        ...mockBlock,
        order: 3,
      });
  ...
  describe('updateBlock', () => {
    it('should update a block when user is editor', async () => {
      ...
      jest.spyOn((prismaService as any).articleBlock, 'findUnique').mockResolvedValue({
        ...mockBlock,
        article: { editorId: mockUserId },
      });

      jest.spyOn((prismaService as any).articleBlock, 'update').mockResolvedValue({
        ...mockBlock,
        content: 'Updated content',
        isBold: true,
      });
    ...
    it('should throw NotFoundException if block does not exist', async () => {
      ...
      jest.spyOn((prismaService as any).articleBlock, 'findUnique').mockResolvedValue(null);
  ...
  describe('deleteBlock', () => {
    it('should delete a block and reorder remaining blocks', async () => {
      ...
      jest.spyOn((prismaService as any).articleBlock, 'delete').mockResolvedValue(mockBlock);
      ...
      const result = await service.deleteBlock(mockBlockId, mockUserId, Role.EDITOR);
  ...
  describe('getArticleWithBlocks', () => {
    ...
    it('should throw NotFoundException if article does not exist', async () => {
      ...
      await expect(
        service.getArticleWithBlocks(mockArticleId),
      ).rejects.toThrow(NotFoundException);
    });
