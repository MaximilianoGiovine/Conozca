...
import { Role, PostStatus, BlockType, FontFamily, TextAlign } from '@conozca/database';
...
describe('Articles Blocks E2E', () => {
  ...
  beforeAll(async () => {
    ...
    if (![200, 201].includes(loginResponse.status)) {
      throw new Error(`Failed to login: ${loginResponse.status} - ${JSON.stringify(loginResponse.body)}`);
    }
