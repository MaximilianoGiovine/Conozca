"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppModule = void 0;
const common_1 = require("@nestjs/common");
const core_1 = require("@nestjs/core");
const config_1 = require("@nestjs/config");
const throttler_1 = require("@nestjs/throttler");
const schedule_1 = require("@nestjs/schedule");
const app_controller_1 = require("./app.controller");
const app_service_1 = require("./app.service");
const prisma_service_1 = require("./prisma.service");
const audit_log_service_1 = require("./common/audit-log.service");
const logger_service_1 = require("./common/logger.service");
const logger_middleware_1 = require("./common/logger.middleware");
const email_service_1 = require("./common/email.service");
const upload_service_1 = require("./common/upload.service");
const sentry_service_1 = require("./common/sentry.service");
const sentry_interceptor_1 = require("./common/sentry.interceptor");
const auth_module_1 = require("./auth/auth.module");
const article_module_1 = require("./articles/article.module");
const upload_module_1 = require("./common/upload.module");
const comment_module_1 = require("./comments/comment.module");
let AppModule = class AppModule {
    configure(consumer) {
        consumer.apply(logger_middleware_1.LoggerMiddleware).forRoutes("*");
    }
};
exports.AppModule = AppModule;
exports.AppModule = AppModule = __decorate([
    (0, common_1.Module)({
        imports: [
            config_1.ConfigModule.forRoot({ isGlobal: true }),
            throttler_1.ThrottlerModule.forRoot({
                throttlers: [{ name: "global", ttl: 60, limit: 100 }],
            }),
            schedule_1.ScheduleModule.forRoot(),
            auth_module_1.AuthModule,
            article_module_1.ArticleModule,
            upload_module_1.UploadModule,
            comment_module_1.CommentModule,
        ],
        controllers: [app_controller_1.AppController],
        providers: [
            app_service_1.AppService,
            prisma_service_1.PrismaService,
            audit_log_service_1.AuditLogService,
            logger_service_1.LoggerService,
            email_service_1.EmailService,
            upload_service_1.UploadService,
            sentry_service_1.SentryService,
            { provide: core_1.APP_GUARD, useClass: throttler_1.ThrottlerGuard },
            { provide: core_1.APP_INTERCEPTOR, useClass: sentry_interceptor_1.SentryInterceptor },
        ],
    })
], AppModule);
//# sourceMappingURL=app.module.js.map