  ...
  describe("Articles - Create", () => {
    it("should create an article as EDITOR", async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          content:
            "This is a test article content with more than fifty characters to satisfy validation for e2e.",
    ...
    it("should not create an article as regular USER", async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          content:
            "Unauthorized content body that is intentionally long enough to pass validation length requirements.",
          excerpt: "Excerpt",
    ...
    it("should not create an article without authentication", async () => {
      const res = await request(app.getHttpServer()).post("/articles").send({
        ...
        content:
          "Unauthenticated content body that is intentionally long enough to pass validation length requirements.",
        excerpt: "Excerpt",
        featuredImage: "image.jpg",
        status: PostStatus.DRAFT,
        authorId,
    ...
    it("should fail if slug already exists", async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          content:
            "Duplicate slug content body that is intentionally long enough to pass validation length requirements.",
          excerpt: "Excerpt",
          featuredImage: "image.jpg",
          status: PostStatus.DRAFT,
          authorId,
    ...
    beforeAll(async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          content:
            "Update content body that is intentionally long enough to pass validation length requirements.",
          excerpt: "Excerpt",
          featuredImage: "image.jpg",
          status: PostStatus.DRAFT,
          authorId,
    ...
    beforeAll(async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .send({
          ...
          content:
            "View content body that is intentionally long enough to pass validation length requirements.",
          excerpt: "Excerpt",
          featuredImage: "image.jpg",
          status: PostStatus.PUBLISHED,
          authorId,
